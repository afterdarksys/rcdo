...
func runCommandObserved(command string, args []string, stdin io.Reader, stdout, stderr io.Writer, observe func([]string)) int {
	...
	switch command {
	case "rego-test":
		err = runRegoTest(args, stdout, stderr)
	case "rego-check":
		err = runRego(args, stdin, stdout, stderr)
	case "pilot":
		err = runPilot(args, stdout, stderr)
	case "context-acquire":
		err = runContextAcquire(args, stdout, stderr)
	...
	case "kube-explain":
		err = runKubeExplain(args, stdin, stdout, stderr)
	case "changes":
		err = runChanges(args, stdout, stderr)
	case "collect":
		err = runCollect(args, stdout, stderr)
	case "doctor":
		err = runDoctor(args, stdout, stderr)
	case "report-read":
		err = runReportRead(args, stdin, stdout, stderr)
	case "fleet-check":
		err = runFleetCheck(args, stdin, stdout, stderr)
	case "runbook":
		err = runRunbookWalk(args, stdout, stderr)
	case "incident":
